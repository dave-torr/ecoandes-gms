...
// operations calc
    ...
    const roomingListDisp=(theDeparture)=>{
        ...
        let eachRoom=theDeparture.roomingList.map((elem, i)=>
            <React.Fragment key={i}>
                <div className={styles.eachRoomDisplayer}>
                    <div style={{width:"39px"}}> {i+1} </div>
                    <div className={styles.aRoomingDetail} style={{width:"120px", borderLeft:"solid 1px black"}}>
        ...
        if(eachNote.length>0){
            noteDisp=eachNote.map((elem,i)=><React.Fragment key={i}> 
                <div className={styles.eachRoomDisplayer}>
                    <div style={{width:"180px", textAlign:"start"}}>{elem.name}</div>
                    <div style={{borderLeft:"solid 1px black", textTransform:"capitalize"}}> &nbsp; 
        ...
        return(<>
            ...
            <div className={styles.roomingListGrid}>
                <div className={styles.roomingListKEYS}>
                    ...
                    <div style={{width:"120px", borderLeft:"solid 1px black"}}> ROOM TYPE</div>
                    <div style={{width:"180px", borderLeft:"solid 1px black" }}> PAX NAME </div>
            ...
            {eachNote.length>0&&<>
                ...
                <div className={styles.roomingListKEYS}>
                    <div style={{width:"180px", borderLeft:"solid 1px black" }}> PAX NAME </div>
                    <div style={{borderLeft:"solid 1px black" }}>&nbsp; GUEST NOTES </div>
                </div>
                    {noteDisp}
                </div>
            </>}
    ...
    {/* Will we need session on per page level, or just on Navi and it can controll it all???? */}
        {session&&<> 
            ...
            {/* select a departure from available deps (from BE) */}
            {theDeparture? <> 
                {expenseTrig?<> 
                ...
                <div className={styles.secondaryBTN} onClick={()=>setTheDeparture(sampleDeparture)}> Select Sample Dep </div>
            </>}
